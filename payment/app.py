...
def get_user_from_db(user_id: str) -> UserValue | None:
    app.logger.info(f"Getting user: {user_id} from the database")
    try:
        # get serialized data
        entry: bytes = db.get(user_id)
    except redis.exceptions.RedisError:
        # return abort(400, DB_ERROR_STR)
        return {"status": "error", "message": DB_ERROR_STR}
    ...
    return {"status": "success", "user": user_id, "credit": entry.credit}
...
def route_request(ch, method, properties, body):
    ...
    if request["tag"] == "find_user":
        ...
        if user['status'] == 'success':
            response = {"status": "success", "user": request["user_id"], }
        else:
